...
distribution[F_MINMAX] = function(limits, data, values) {
    ...
    if (math.min(replaced) === math.max(replaced)) {
        return replaced.map(function() { return math.mean([limits.min, limits.max]); })
    }

    return replaced.map(
        math.linear( math.min(replaced), limits.min, math.max(replaced), limits.max )
    );
};
...
distribution[F_MAXMIN] = function(limits, data, values) {
    ...
    return replaced.map(
        math.linear( math.min(replaced), limits.max, math.max(replaced), limits.min )
    );
};
